...
@Component({selector: 'app-ajouter-ventes', templateUrl: './ajouter-ventes.component.html', styleUrls: ['./ajouter-ventes.component.scss']})
export class AjouterVentesComponent implements OnInit {
    ...
    constructor(private domSanitizer : DomSanitizer, private matIconRegistry : MatIconRegistry, private fb : FormBuilder, private commercantService : CommercantService, private modalService : NzModalService, private notification : NzNotificationService, private sharedService : SharedService, private route : ActivatedRoute, private notificatoinService : NotificationService, private location : Location) {
        this.matIconRegistry.addSvgIcon('paycash', this.domSanitizer.bypassSecurityTrustResourceUrl('../assets/img/paycash.svg'));
        this.matIconRegistry.addSvgIcon('payonline', this.domSanitizer.bypassSecurityTrustResourceUrl('../assets/img/payonline.svg'));
    }
    ...
    saveVente(mode : 'online' | 'offline', via : 'om' | 'wave' | 'free' | 'local' = 'local') {
        this.isLoad = true;

        this.commercantService.createCommande(this.produits, this.selectedClient.id, this.payementFormGroup.value.interet, this.payementFormGroup.value.firstPart, this.payementFormGroup.value.telephone, this.payementType, via).subscribe({
            next: (response) => {
                console.log(response);
                let type: any = 'info';
                if (mode == 'online') {
                    ...
                    this.notification.success('Notification', 'Commande est validé avec succès');
                    this.location.back();
                    this.isLoad = false;
                }
            },
            error: (errors) => {
                this.notificatoinService.emitChange({title: 'Paiement annullé', message: errors.error.message, type: 'error'});
                this.isLoad = false;
                console.error(errors);
            }
        });
    ...
    toggleMobilePayementModal() {
        this.payementFormGroup.addControl('telephone', new FormControl(this.selectedClient.telephone, [Validators.required, Validators.pattern('^(77|78|75|70|76)[0-9]{7}$'),]));
    ...
    onPayementTypeSelected(name : string) {
        this.payementFormGroup = this.fb.group({
            firstPart: [
                null,
                [
                    Validators.required, Validators.min(this.aPayer())
                ]
            ],
            interet: [
                null,
